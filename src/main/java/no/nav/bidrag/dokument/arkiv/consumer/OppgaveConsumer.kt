...
class OppgaveConsumer(restTemplate: RestTemplate?) : AbstractConsumer(restTemplate) {
    fun finnOppgaver(parametre: OppgaveSokParametre): OppgaveSokResponse? {
        ...
        return restTemplate.exchange(pathMedParametre, HttpMethod.GET, null, OppgaveSokResponse::class.java).body
    }

    fun opprett(opprettOppgaveRequest: OpprettOppgaveRequest): Long? {
        val oppgaveResponse = restTemplate.postForEntity("/", opprettOppgaveRequest, OppgaveResponse::class.java)
    ...
    fun patchOppgave(oppgavePatch: OppgaveData): OppgaveData? {
        ...
        return restTemplate.patchForObject("/${oppgavePatch.id}", oppgavePatch, OppgaveData::class.java)
    }
