...
public class DokdistFordelingConsumer {
      ...
      if (HttpStatus.BAD_REQUEST.equals(status) || HttpStatus.NOT_FOUND.equals(status)) {
        throw new DistribusjonFeiletFunksjoneltException(
            String.format(
                ...
                journalpostId, e.getStatusCode(), errorMessage));
      }
  ...
  private DistribuerJournalpostResponse conflictExceptionToResponse(
      Long journalpostId, HttpStatusCodeException e) {
    try {
      var response =
