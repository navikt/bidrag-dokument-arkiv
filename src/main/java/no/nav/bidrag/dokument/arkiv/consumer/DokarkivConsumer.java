...
public class DokarkivConsumer extends AbstractConsumer {
  ...
  public static final String URL_JOURNALPOSTAPI_V1_FEILREGISTRER =
      "/rest/journalpostapi/v1/journalpost/%s/feilregistrer";
  ...
  @Retryable(
      value = OppdaterJournalpostFeiletTekniskException.class,
      maxAttempts = 5,
  ...
  public OppdaterJournalpostResponse endre(OppdaterJournalpostRequest oppdaterJournalpostRequest) {
    var oppdaterJoarnalpostApiUrl =
        URL_JOURNALPOSTAPI_V1 + '/' + oppdaterJournalpostRequest.hentJournalpostId();
    try {
      var response =
          restTemplate.exchange(
              oppdaterJoarnalpostApiUrl,
              HttpMethod.PUT,
              new HttpEntity<>(oppdaterJournalpostRequest),
              OppdaterJournalpostResponse.class);
      LOGGER.info(
          "Endret journalpost {} med respons {}",
          oppdaterJournalpostRequest.hentJournalpostId(),
      ...
      if (HttpStatus.BAD_REQUEST.equals(status) || HttpStatus.NOT_FOUND.equals(status)) {
        throw new OppdaterJournalpostFeiletFunksjoneltException(
            String.format(
                "Oppdatering av journalpost %s feilet med status %s og feilmelding: %s",
                oppdaterJournalpostRequest.hentJournalpostId(), e.getStatusCode(), errorMessage));
      }
      throw new OppdaterJournalpostFeiletTekniskException(
          String.format(
              "Oppdatering av journalpost %s feilet med status %s og feilmelding: %s",
              oppdaterJournalpostRequest.hentJournalpostId(), e.getStatusCode(), errorMessage),
          e);
  ...
  public JoarkOpprettJournalpostResponse opprett(
      JoarkOpprettJournalpostRequest joarkOpprettJournalpostRequest, boolean ferdigstill) {
    try {
      SECURE_LOGGER.info("Oppretter journalpost {}", joarkOpprettJournalpostRequest);
      var response =
          restTemplate.exchange(
              URL_JOURNALPOSTAPI_V1
                  + String.format("?forsoekFerdigstill=%s", ferdigstill ? "true" : "false"),
              HttpMethod.POST,
              new HttpEntity<>(joarkOpprettJournalpostRequest),
              JoarkOpprettJournalpostResponse.class);
      var responseBody = response.getBody();
      LOGGER.info(
          "Opprettet journalpost {} med status {}",
          responseBody.getJournalpostId(),
          responseBody.getJournalstatus());
      return response.getBody();
    } catch (HttpClientErrorException clientErrorException) {
      if (clientErrorException.getStatusCode() == HttpStatus.CONFLICT) {
        LOGGER.info(
            "Journalpost med eksternReferanseId {} er allerede arkivert i Joark",
            joarkOpprettJournalpostRequest.getEksternReferanseId());
  ...
  public HttpResponse<Void> ferdigstill(
      FerdigstillJournalpostRequest ferdigstillJournalpostRequest) {
    var oppdaterJoarnalpostApiUrl =
        URL_JOURNALPOSTAPI_V1
            + '/'
            + ferdigstillJournalpostRequest.getJournalpostId()
            + "/ferdigstill";
    var response =
        restTemplate.exchange(
            oppdaterJoarnalpostApiUrl,
            HttpMethod.PATCH,
            new HttpEntity<>(ferdigstillJournalpostRequest),
            Void.class);
  ...
  public HttpResponse<Void> feilregistrerSakstilknytning(Long journalpostId) {
    try {
      var oppdaterJoarnalpostApiUrl =
          String.format(
              URL_JOURNALPOSTAPI_V1_FEILREGISTRER + "/feilregistrerSakstilknytning", journalpostId);
      var response =
      ...
      var erSakstilknytningAlleredeFeilregistrert =
  ...
  public HttpResponse<Void> oppdaterDistribusjonsInfo(
      Long journalpostId, boolean settStatusEkspedert, JournalpostUtsendingKanal utsendingsKanal) {
    var endpoint = URL_JOURNALPOSTAPI_V1 + "/%s";
    var oppdaterJoarnalpostApiUrl =
        String.format(endpoint + "/oppdaterDistribusjonsinfo", journalpostId);
    var request = new OppdaterDistribusjonsInfoRequest(settStatusEkspedert, utsendingsKanal);
    try {
      var response =
          restTemplate.exchange(
              oppdaterJoarnalpostApiUrl, HttpMethod.PATCH, new HttpEntity<>(request), Void.class);
      return new HttpResponse<>(response);
    } catch (HttpClientErrorException clientErrorException) {
      ...
      if (clientErrorException.getStatusCode() == HttpStatus.BAD_REQUEST
          && clientErrorException
              ...
              .contains("Kan ikke ekspedere journalpost med status E")) {
        LOGGER.warn(
            "Sett status til EKSPEDERT for journalpost {} feilet med melding {}",
            journalpostId,
  ...
  public HttpResponse<Void> opphevFeilregistrerSakstilknytning(Long journalpostId) {
    var oppdaterJoarnalpostApiUrl =
        String.format(
            URL_JOURNALPOSTAPI_V1_FEILREGISTRER + "/opphevFeilregistrertSakstilknytning",
            journalpostId);
    var response =
        restTemplate.exchange(oppdaterJoarnalpostApiUrl, HttpMethod.PATCH, null, Void.class);
    return new HttpResponse<>(response);
  ...
  private JoarkOpprettJournalpostResponse handleConflictResponse(
