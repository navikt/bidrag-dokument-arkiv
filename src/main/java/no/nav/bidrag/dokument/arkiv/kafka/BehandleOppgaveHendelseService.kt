...
class BehandleOppgaveHendelseService(
    ...
    private fun opprettEkstraKommentarSomLeggesTilOppgave(journalpost: Journalpost): String? {
        SECURE_LOGGER.info("Journalpost kommet retur. Sjekker detaljer ${journalpost.journalpostId} ${journalpost.journalstatus} ${journalpost.distribuertTilAdresse()}")
        return if (journalpost.journalstatus == JournalStatus.JOURNALFOERT || journalpost.distribuertTilAdresse() == null) "Mottaker har ikke åpnet forsendelsen via www.nav.no innen 40 timer. Ingen postadresse er registrert. Vurder om mottaker har adresse forsendelsen kan sendes til." else null
    }
}
