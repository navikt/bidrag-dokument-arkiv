...
internal class LagreJournalpostTest {
    ...
    fun `Skal lagre journalpost endret returdetaljer`() {
        ...
        journalpost.antallRetur = 1
        val endreJournalpostCommand = createEndreJournalpostCommand(
            listOf(EndreReturDetaljer(datoSomSkalEndres, LocalDate.parse("2022-11-15"), "Ny beskrivelse 1"))
        )
        ...
        assertThat(
            lagreJournalpost.tilleggsopplysninger?.filter { it.values.any { value -> value.contains("2022-11-15") } }
                ?.joinToString()
